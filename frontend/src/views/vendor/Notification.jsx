...
function Notification() {
  ...
  const fetchNoti = async () => {
    await apiInstance(`vendor-noti-list/${userData?.vendor_id}/`).then(
      (res) => {
  ...
  const fetchNotiStats = async () => {
    await apiInstance(`vendor-noti-summary/${userData?.vendor_id}/`).then(
      (res) => {
        setStats(res.data[0]);
      }
    );
  ...
  const markAsSeen = async (notiId) => {
    await apiInstance(
      `vendor-noti-mark-as-seen/${userData?.vendor_id}/${notiId}/`
    ).then((res) => {
  ...
  const handleFilterNoti = async (filter) => {
    const response = await apiInstance.get(
      `vendor/noti/filter/${userData?.vendor_id}?filter=${filter}`
    );
            ...
            <div className="col-lg-12">
              ...
              <div className="dropdown">
                ...
                >
                  <li>
                    <a
                      onClick={() => handleFilterNoti("latest")}
                  ...
                  <li>
                    <a
                      onClick={() => handleFilterNoti("oldest")}
                      className="dropdown-item"
                      href="#"
                    >
                      Date: Oldest
                  ...
                  <li>
                    <a
                      onClick={() => handleFilterNoti("unread")}
                      className="dropdown-item"
                      href="#"
                    >
                      Status: UnRead
