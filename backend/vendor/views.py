...
from store.models import  Product,CartOrder,CartOrderItem,Review,Coupon,Notification
from vendor.models import  Vendor
from userauths.models import User,Profile

from store.serializer import  CartOrderSerializer, SummarySerializer,ProductSerializer,CartOrderItemSerializer,EarningSerializer,ReviewSerializer,CouponSerializer,CouponSummarySerializer,NotificationSerializer,NotificationSummarySerializer, VendorSerializer,SpecificationSerializer,ColorSerializer,SizeSerializer,GallerySerializer
...
class FilterNotificationAPIView(generics.ListAPIView):
    serializer_class=NotificationSerializer
    permission_classes=[AllowAny]
    ...
    def get_queryset(self):
        ...
        if filter =="read":
            noti=Notification.objects.filter(vendor=vendor, seen=True)
        elif filter =="unread":
            noti=Notification.objects.filter(vendor=vendor, seen=False)
        elif filter=='latest':
            noti=Notification.objects.filter(vendor=vendor).order_by('-id')
        elif filter=='oldest':
            noti=Notification.objects.filter(vendor=vendor).order_by('id')

        else:
            noti=Notification.objects.filter(vendor=vendor)
        return noti
